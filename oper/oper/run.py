...
class Run:
    ...
    def _get(self, hierarchy, time, merged, interp, hier_type, all_primal):
        """
        ...
        if merged:
            ...
            # print('xxxx ', dir(hierarchy))
            # print('xxxx ', dir(hierarchy.levels()))
            # return hierarchy
            if hier_type == "vector":
                return VectorField(hierarchy.patch_levels, hierarchy.domain_box, time=time, all_primal=all_primal)

    def GetTags(self, time, merged=False):
        hier = self._get_hierarchy(time, "tags.h5")
        return self._get(hier, time, merged, "nearest")

    def GetB(self, time, merged=False, interp='nearest', all_primal=False):
        print("1")
        hier = self._get_hierarchy(time, "EM_B.h5")
        print('xxxx ', dir(hier))
        print("2")
        return self._get(hier, time, merged, interp, "vector", all_primal)

    def GetE(self, time, merged=False, interp='nearest'):
        hier = self._get_hierarchy(time, "EM_E.h5")
        return self._get(hier, time, merged, interp, "vector")
