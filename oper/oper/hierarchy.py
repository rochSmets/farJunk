...
class FieldData(PatchData):
    ...
    def all_primal(self, name):
        ds = self.dataset
        ds_shape = list(ds.shape)
        for i in range(self.ndim):
            if self.centerings[i] == 'dual':
                ds_shape[i] = self.dataset.shape[i]+1

        ds_all_primal = np.full(ds_shape, np.nan)

        nb_ghosts = self.layout.nbrGhosts(self.layout.interp_order, 'primal')
        ...
        for i, centering in enumerate(self.centerings):
            if centering == 'dual':
                lo[i] = [ nb_ghosts-1,  nb_ghosts  ]
                up[i] = [-nb_ghosts  , -nb_ghosts+1]
                we[i] = [0.5, 0.5]
            else:
                lo[i] = [ nb_ghosts]
                up[i] = [-nb_ghosts]
        ...
        if self.ndim == 1:
            ...
            ds_all_primal[nb_ghosts:-nb_ghosts] = weighted_sum(self.dataset, lowers, uppers, weights)
        ...
        if self.ndim == 2:
            ...
            ds_all_primal[nb_ghosts:-nb_ghosts,\
                          nb_ghosts:-nb_ghosts] = weighted_sum(self.dataset, lowers, uppers, weights)
        ...
        # print('qqqq ', name)
        zob = FieldData(self.layout, name, ds_all_primal, centering=['primal']*self.box.ndim)

        # print(dir(zob))
        # print('gggg ', name)
        # print('gggg ', zob.centerings)
        # print('gggg ', zob.dataset.shape)

        return FieldData(self.layout, name, ds_all_primal, centering=['primal']*self.box.ndim)
...
class PatchHierarchy(object):
    ...
    def is_vector_hierarchy(self):
        print("sss ", self.__dict__.keys())
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ### all_primal = False
        ### if len(self.quantities()) == 1 and self.quantities()[0] in list(field_qties.values()):
        ###     qty = self.quantities()[0]
        ###     all_primal = kwargs.get("all_primal", True)
        ###     # print('rrrr ', self.flat_name(qty))
        ...
        ###                 # print('xxxx ', qty, patch.patch_datas[qty].centerings)
        ###                 # print('xxxx ', qty, patch.patch_datas[qty].dataset.shape)
        ###                 if 'dual' in patch.patch_datas[qty].centerings:
        ###                     pd_all_primal = patch.patch_datas[qty].all_primal(self.flat_name(qty))
        ###                     print('nnnn id : ', patch.id)
        ###                     patches += [Patch({self.flat_name(qty): pd_all_primal}, patch.id)]
        ###                 # TODO : pas besoin de else... le dict de la patchHierarchy est déja rempli
        ###                 # le 'ifi' ci-dessus sert juste a updater les patch conternant du dual

        ###             # for patch in patches:
        ###             #     print('~~~~ ', qty, self.flat_name(qty), patch.patch_datas[self.flat_name(qty)].centerings)
        ###             #     print('++++ ', dir(patch.patch_datas[self.flat_name(qty)]))

        ###             new_lvls[ilvl] = PatchLevel(ilvl, patches)

        ###         if new_lvls:
        ###             print('---- ', new_lvls)
        ###             self.time_hier.update({self.format_timestamp(time): new_lvls})
        ...
        ###             #     print("centerings : {}".format(pdata.centerings))
...
class VectorField(PatchHierarchy):
    ...
    def __init__(self, patch_levels, domain_box, **kwargs):
        ...
        # TODO : refaire un patch levels sur primal...
        # un patch_level est forcement a un seul temps donne
        # mais ensuite, une patchHierarchy peut en contenir plusieurs...

        # boucler sur les levels, sur les patch, sur les patchdata, et tout mettre sur primal !
        # ensuite donner ce patch_levels au ctor de PatchHierarchy

        # print(type(patch_levels))
        # print(patch_levels.items())

        super().__init__(patch_levels, domain_box, refinement_ratio, time, data_files) # TODO names=names



    def onSameCenteringAs(self, vecfield):

        assert(isinstance(vecfield, VectorField))
        assert(len(list(self.time_hier.keys())) == len(list(vecfield.time_hier.keys())))


        times_left = list(self.time_hier.keys())
        times_right = list(vecfield.time_hier.keys())

        assert(all(l == r for l, r in zip(times_left, times_right)))

        for (time_left, time_right) in zip(list(self.time_hier.keys()), list(vecfield.time_hier.keys())):
            new_patch_level = {}

            lvls_left = self.levels(time_left)
            lvls_right = self.levels(time_right)

            assert(all (l == r for l, r in zip(lvls_left.keys(), lvls_right.keys())))

            for (ilvl_left, lvl_left), (ilvl_right, lvl_right) in zip(lvls_left.items(), lvls_right.items()):
                new_patches = {}
                ...
                for (patch_left, patch_right) in zip(lvl_left.patches, lvl_right.patches):
                    ...
                    for (name, name_left, name_right) in zip(self.names, patch_left.patch_datas.keys(), patch_right.patch_datas.keys()):
                        pd_left = patch_left.patch_datas[name_left]
                        pd_right = patch_right.patch_datas[name_right]

                        if pd_left.centerings == pd_right.centerings:
                             new_pd[name] = pd_left

                    if ilvl_left not in new_patches:
                        new_patches[ilvl_left] = []

                    new_patches[ilvl_left].append(Patch(new_pd, l.id))
...
# def flat_name(hier): # TODO
#     print(type(hier))
#     if isinstance(hier, VectorField):
#         return hier.names
...
def amr_grid(hierarchy, time):
    """returns a non-uniform contiguous primal grii
